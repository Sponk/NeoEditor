...
Vector3 Matrix4x4::getEulerAngles(void) const
{
...
Vector3 Matrix4x4::getScale(void) const
{
	...
	float ySize = getRotatedVector3(Vector3(0, 1, 0)).getLength();
	float zSize = getRotatedVector3(Vector3(0, 0, 1)).getLength();
...
void Matrix4x4::operator *= (const float value)
{
	(*this) = (*this) * value;
}
...
Vector3 Matrix4x4::getInverseRotatedVector3(const Vector3 & vec) const
{
	return Vector3(
...
Vector3 Matrix4x4::getTranslatedVector3(const Vector3 & vec) const
{
	return Vector3(vec.x + entries[12], vec.y + entries[13], vec.z + entries[14]);
}

Vector3 Matrix4x4::getInversetranslatedVector3(const Vector3 & vec) const
{
	return Vector3(vec.x - entries[12], vec.y - entries[13], vec.z - entries[14]);
}
...
void Matrix4x4::setUniformScale(const float size)
{
	loadIdentity();
	entries[0] = entries[5] = entries[10] = size;
}

void Matrix4x4::scale(const Vector3 & scaleFactor)
{
	entries[0] *= scaleFactor.x;
...
void Matrix4x4::translate(const Vector3 translate)
{
	Matrix4x4 matrix;
