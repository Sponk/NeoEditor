...
static const char* textFragShader = //"#version 130\n"
...
void GL3Renderer::initialize()
{
	...
	// Load color only shader
	{
		...
		m_objectShader = loadShader((shaderpath + "/gl3.vert").c_str(), (shaderpath + "/gl3.frag").c_str());
	}
...
int GL3Renderer::findLights(Neo::Scene* scene, Neo::OEntity* entity, Neo::SubMesh* submesh)
{
	int num = 0;
...
void GL3Renderer::sendLight(unsigned int fx, const char* prefix, OLight* l, int num, const Matrix4x4& matrix)
{
	char str[255];
	char ending[255];
...
void GL3Renderer::drawText(OText* textObj, OCamera* camera)
{
...
void GL3Renderer::drawText2D(OText* text, float x, float y, float rotation)
{
...
void GL3Renderer::drawEntity(OEntity* e, OCamera* camera)
{
	drawEntity(e, nullptr, camera);
}

void GL3Renderer::drawEntity(OEntity* e, Scene* scene, OCamera* camera)
{
	Mesh* mesh = e->getMesh();
...
void GL3Renderer::drawSubmeshDisplay(SubMesh* submesh, MaterialDisplay* display, PRIMITIVE_TYPES primtype)
{
...
void GL3Renderer::prepareMaterialDisplay(unsigned int fx, SubMesh* mesh, MaterialDisplay* display)
{
...
void GL3Renderer::initVBO(SubMesh* subMesh)
{
	NeoEngine* engine = NeoEngine::getInstance();
...
void GL3Renderer::clearScreen(const Vector3& color)
{
	glClearColor(color.x, color.y, color.z, 0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

void GL3Renderer::set2D(const Vector2& size)
{
	set2D(size.x, size.y);
}

void GL3Renderer::set2D(unsigned int w, unsigned int h)
{
	setOrthoView(m_matrix, 0.0f, w, h, 0.0f, 1.0f, -1.0f);
}

void GL3Renderer::drawTexturedQuad(const Vector2& position, const Vector2& size, int texture,
...
void GL3Renderer::drawColoredQuad(const Vector2& position, const Vector2& size, const Vector4& color, float rotation)
{
	glEnable(GL_BLEND);
	glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
...
void GL3Renderer::sendTexture(unsigned int id, Image* image, bool mipMap, bool filter, bool compress)
{
...
unsigned int GL3Renderer::createTexture()
{
	unsigned int tex;
	glGenTextures(1, &tex);
	return tex;
...
void GL3Renderer::destroyTexture(unsigned int id)
{
	glDeleteTextures(1, &id);
}

void GL3Renderer::drawTexturedQuad(const Vector2& position, const Vector2& size, int texture, float rotation)
{
...
unsigned int GL3Renderer::createShader(const char* vertexSource, const char* fragmentSource)
{
	...
	if(!reportShaderError(vert, "vertex")) 
	{
		MLOG_ERROR(vertexSource);
		return 0;
	}
	...
	if(!reportShaderError(frag, "fragment"))
	{
		MLOG_ERROR(fragmentSource);
		return 0;
	}
...
unsigned int GL3Renderer::loadShader(const char* vertPath, const char* fragPath)
{
	// FIXME: Preprocessor!
	char* vert, *frag;
