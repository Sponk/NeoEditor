...
void loadSettings(const char* path)
{
	...
	Fl::background(static_cast<unsigned char>(vector.x),
	...
	Fl::background2(static_cast<unsigned char>(vector.x),
					static_cast<unsigned char>(vector.y),
					static_cast<unsigned char>(vector.z));
	...
	Fl::foreground(static_cast<unsigned char>(vector.x),
				   static_cast<unsigned char>(vector.y),
				   static_cast<unsigned char>(vector.z));
	...
	main_window->resize(STR1_TO_FLOAT(parser.top()("window")["xpos"]),
						STR1_TO_FLOAT(parser.top()("window")["ypos"]), width,
						height);

	translation_speed =
		STR1_TO_FLOAT(parser.top()("window")["translationSpeed"]);
...
int main(int argc, char** argv)
{
	...
	MLOG_INFO("SDL compiled version:\t" << (int)compiled.major << "."
										<< (int)compiled.minor << "."
										<< (int)compiled.patch);
	MLOG_INFO("SDL linked version:\t" << (int)linked.major << "."
									  << (int)linked.minor << "."
									  << (int)linked.patch);
