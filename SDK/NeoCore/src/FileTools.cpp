...
bool copyFile(const char * inFilename, const char * outFilename)
{
    ...
    File * out = M_fopen(outFilename, "wb");
    if(! out)
    {
...
FileOpenHook* M_getFileOpenHook()
{
...
size_t M_fread(void * dest, size_t size, size_t count, File * stream)
{
    if(stream)
...
size_t M_fwrite(const void * str, size_t size, size_t count, File * stream)
{
    if(stream)
...
int M_fprintf(File * stream, const char * format, ...)
{
    va_list args;
    if(stream)
    {
...
int M_fseek(File * stream, long offset, int whence)
{
    if(stream)
