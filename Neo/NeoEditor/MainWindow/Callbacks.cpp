...
// Save settings
	...
	{
		out << "[lang]" << endl
			<< "name=" << Translator::getInstance()->getLanguageFile();
		out.close();
	}
}
...
void window_quit(Fl_Double_Window*, void*) { quit_callback(NULL, NULL); }
	...
	if (entity)
	{
		...
		for (int i = 0; i < entity->getChildrenNumber(); i++)
		{
			...
			if (!window.scene_tree->find_item(
					(path + "/" + std::string(object->getName())).c_str()))
			{
				Fl_Tree_Item* item = window.scene_tree->add(
					(path + "/" + std::string(object->getName())).c_str());
			...
			add_to_tree(scene->getObjectByName(entity->getChild(i)->getName()),
						path.c_str());
		}
	}
}
...
void update_scene_tree()
{
	...
	for (int i = 0; i < level->getScenesNumber(); i++)
	{
		...
		window.scenes_menu->add(scene->getName(), 0,
								(Fl_Callback*)change_scene_callback,
								(void*)index);
	}
...
void open_project_callback(Fl_Menu_*, void*)
{
	...
	if (!current_project.path.empty() &&
		!ask(tr(
			"You need to close the current project. Do you want to proceed?")))
		return;
	...
	const char* filename =
		fl_native_file_chooser(tr("Open project"), "*.mproj", home,
							   Fl_Native_File_Chooser::BROWSE_FILE);
...
#ifndef WIN32
		current_project.path = current_project.path.erase(
			current_project.path.find_last_of("/") + 1,
...
void* get_variable_pointer(const char* bname, const char* vname,
						   Object3d* object)
{
...
void behavior_float_callback(Fl_Value_Input* input, const char* data)
{
	...
	Object3d* object = NeoEngine::getInstance()
						   ->getLevel()
						   ->getCurrentScene()
						   ->getObjectByName(window.name_edit->value());

	if (!object || !input || !data)
		return;

	*((float*)get_variable_pointer(data, input->label(), object)) =
		input->value();
}
...
void behavior_vector4_callback(Fl_Value_Input* input, const char* data)
{
	...
	if (!group)
	{
		MLOG_WARNING("Could not update behavior! The input does not belong to "
					 "any group!");
		return;
	}
...
void create_behavior_ui(Object3d* object)
{
	...
	for (int b = 0; b < num_behaviors; b++)
	{
		...
		Fl_Group* group =
			new Fl_Group(window.behaviors_scroll->x() + 4,
						 window.behaviors_scroll->y() + height + 20, 218, 0);
		...
		for (int i = 0; i < num_variables; i++)
		{
			...
			switch (var.getType())
			{
				...
				{
					Fl_Value_Input* input = new Fl_Value_Input(
						x + 5, y + height + 20, 90, 22, var.getName());
					input->align(Fl_Align(FL_ALIGN_TOP_LEFT));
					input->value(*(float*)var.getPointer());

					input->callback((Fl_Callback*)behavior_float_callback,
				...
				case M_VARIABLE_INT:
				{
					...
					input->callback((Fl_Callback*)behavior_int_callback,
									(void*)behavior->getName());
				...
				{
					Fl_Group* vec_group = new Fl_Group(x + 5, y + height + 20,
													   205, 30, var.getName());
					...
					for (int i = 0; i < 3; i++)
					{
						Fl_Value_Input* input =
							new Fl_Value_Input(x + 65 * i + 7, y + height + 30,
											   65, 22, &labels[i][0]);
						input->align(Fl_Align(FL_ALIGN_TOP_LEFT));
						input->value(values[i]);

						input->callback((Fl_Callback*)behavior_vector3_callback,
										(void*)var.getName());
				...
				case M_VARIABLE_VEC2:
				{
					...
					for (int i = 0; i < 2; i++)
					{
						...
						input->callback((Fl_Callback*)behavior_vector2_callback,
										(void*)var.getName());
				...
				case M_VARIABLE_VEC4:
				{
					...
					{
						Fl_Value_Input* input =
							new Fl_Value_Input(x + 50 * i + 7, y + height + 30,
											   50, 22, &labels[i][0]);
						input->align(Fl_Align(FL_ALIGN_TOP_LEFT));
						input->value(values[i]);

						input->callback((Fl_Callback*)behavior_vector4_callback,
										(void*)var.getName());
				...
				case M_VARIABLE_BOOL:
				{
					Fl_Check_Button* button = new Fl_Check_Button(
						x + 5, y + height + 20, 205, 22, var.getName());
					button->value(*(bool*)var.getPointer());

					button->callback((Fl_Callback*)behavior_bool_callback,
									 (void*)behavior->getName());
		...
		Fl_Button* remove_button =
			new Fl_Button(x + 5, y + height, 205, 22, "Remove");
		remove_button->callback((Fl_Callback*)remove_behavior,
								(void*)(object->getBehaviorsNumber() - 1));
...
void scene_tree_callback(DnDTree* tree, long update_tree)
{
	...
	if (object->getType() == M_OBJECT3D_ENTITY)
	{
		...
		if (object_window == NULL)
		{
			...
			object_window->position(window.special_tab->x() + 3,
									window.special_tab->y() + 3);
		...
		if (phys)
		{
			...
			window.object_angular_damping_edit->value(
	...
	else if (object->getType() == M_OBJECT3D_LIGHT)
	{
		...
		if (light_window == NULL)
		{
			...
			light_window->position(window.special_tab->x() + 3,
								   window.special_tab->y());
			light_window->size(window.special_tab->w(),
							   window.special_tab->h());
	...
	else if (object->getType() == M_OBJECT3D_CAMERA)
	{
		...
		if (camera_window == NULL)
		{
			...
			camera_window->position(window.special_tab->x() + 3,
									window.special_tab->y());
	...
	else if (object->getType() == M_OBJECT3D_TEXT)
	{
		...
		if (text_window == NULL)
		{
			...
			text_window->position(window.special_tab->x() + 3,
								  window.special_tab->y());
		...
		if (update_name)
		{
			window.text_text_edit->buffer()->remove(
				0, window.text_text_edit->buffer()->length());
	...
	else if (object->getType() == M_OBJECT3D_SOUND)
	{
		...
		if (sound_window == NULL)
		{
			...
			sound_window->position(window.special_tab->x() + 3,
								   window.special_tab->y());
...
void edit_name_callback(Fl_Input*, void*)
{
	...
	{
		parent = NeoEngine::getInstance()
					 ->getLevel()
					 ->getCurrentScene()
					 ->getObjectByName(window.parent_edit->value());
...
{
	OLight* light =
		NeoEngine::getInstance()->getLevel()->getCurrentScene()->getLightByName(
			window.name_edit->value());

	if (!light)
	{
		MLOG_WARNING("Light with the name '" << window.name_edit->value()
											 << "' does not exist!");
		return;
	}
...
void publish_callback(Fl_Menu_*, void*)
{
	if (current_project.path.empty())
	{
		...
		fl_message(
			tr("Can't publish: You need to create/open a project first!"));
		return;
	}
...
void save_level_callback(Fl_Menu_*, long mode)
{
	...
	NeoGame* game = NeoEngine::getInstance()->getGame();
	game->getPostProcessor()->setShaderPath(
...
{
	const char* filename =
		fl_native_file_chooser(tr("Select project file"), "*.mproj", NULL,
							   Fl_Native_File_Chooser::BROWSE_SAVE_FILE);
	...
	current_project.path =
		current_project.path.erase(current_project.path.find_last_of("/") + 1,
								   current_project.path.length());
#else
	current_project.path =
		current_project.path.erase(current_project.path.find_last_of("\\") + 1,
...
{
	OEntity* entity = NeoEngine::getInstance()
						  ->getLevel()
						  ->getCurrentScene()
						  ->getEntityByName(window.name_edit->value());

	if (!entity || !entity->getPhysicsProperties())
		return;

	entity->getPhysicsProperties()->setCollisionShape(
...
void edit_camera_properties(Fl_Value_Input*, void*)
{
	OCamera* camera = NeoEngine::getInstance()
						  ->getLevel()
						  ->getCurrentScene()
						  ->getCameraByName(window.name_edit->value());
...
void text_find_font_callback(Fl_Button*, void*)
{
	...
	Level* level = NeoEngine::getInstance()->getLevel();
	OText* text =
		level->getCurrentScene()->getTextByName(window.name_edit->value());
...
void play_game_callback(Fl_Menu_*, void*)
{
...
#ifndef WIN32
	std::string project_name = current_project.file_path.substr(
		current_project.file_path.find_last_of("/") + 1);
	...
	if (!isFileExist((current_project.path + "NeoPlayer").c_str()))
	{
		...
		fl_alert(
			tr("Could not start player! Make sure that the executable exists "
			   "and you have rights to start it.\nIf it does not exist, try "
			   "the 'Update player' option in the project menu."));
		return;
	}

	errno = 0;
	FILE* file = popen(("\"" + current_project.path + "NeoPlayer\" \"" +
						project_name + "\" 1024 768 0 1 2>&1").c_str(),
					   "r");
#else
	std::string project_name = current_project.file_path.substr(
		current_project.file_path.find_last_of("\\") + 1);
	...
	errno = 0;
	FILE* file = _popen(("\"" + current_project.path + "NeoPlayer.exe '" +
						 project_name + "' 1024 768 0 1\" 2>&1").c_str(),
						"r");
#endif
...
void add_behavior_menu_callback(Fl_Menu_* menu, const char* name)
{
	...
	Object3d* object = engine->getLevel()->getCurrentScene()->getObjectByName(
		window.name_edit->value());
...
void update_behavior_menu()
{
	...
	for (int i = 0; i < manager->getBehaviorsNumber(); i++)
	{
		...
		window.menu_bar->add(name.c_str(), 0,
							 (Fl_Callback*)add_behavior_menu_callback,
							 (void*)behavior->getName(), 0);
	}
}
...
void import_mesh_callback(Fl_Menu_*, void*)
{
	const char* filename = fl_native_file_chooser(
		tr("Choose file"), "*.{obj,dae,3ds,b3d,ase,ifc,xgl,zgl,lwo,stl,x,ms3d,"
						   "cob,irrmesh,md1,md2,md3,mdc,md5,ter}",
...
{
	OSound* sound =
		NeoEngine::getInstance()->getLevel()->getCurrentScene()->getSoundByName(
			window.name_edit->value());
...
void scene_setup_callback(Fl_Menu_*, void*)
{
	...
	{
		fl_message(
			tr("You need to save the level before editing scene properties!"));
		return;
	}
...
void delete_scene_callback(Fl_Menu_*, void*)
{
	...
	if (level->getScenesNumber() - 1 <= 0)
	{
		...
		fl_alert(
			tr("Can not delete scene! The level needs at least one scene!"));
		return;
	}
...
void about_menu_callback(Fl_Menu_*, void*)
{
	...
	window->position(main_window->x() + 0.5 * main_window->w() -
						 0.5 * window->w(),
					 main_window->y() + 0.33 * main_window->h());
	window->show();
}
...
void setCursorPos(int x, int y)
{
	...
	XWarpPointer(fl_display, rootwindow, rootwindow, 0, 0, 0, 0,
				 ::window.glbox->x_root() + x, ::window.glbox->y_root() + y);
#endif
}
...
int check_updates_thread(void* data)
{
	...
	const char* versiontxt =
		connection->sendGetRequest("/downloads/neo/daily/version.txt");
...
void check_for_updates_callback(Fl_Menu_*, void*)
{
	...
	if (ret == 1)
	{
		...
		fl_message(
			tr("Could not retrieve version information from the server!"));
		return;
	}
	...
	MLOG_INFO("Official version is " << version << ", current version is "
									 << EDITOR_VERSION_STRING);
	...
	{
		fl_message(tr("You already run the latest and greatest version of this "
					  "application!"));
	}
