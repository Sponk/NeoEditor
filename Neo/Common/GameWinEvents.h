...
#ifndef __GAME_WIN_EVENTS_H
#define __GAME_WIN_EVENTS_H
...
void gameWinEvents(MWinEvent * windowEvents)
{
	...
	InputContext * input = Neo::NeoEngine::getInstance()->getInputContext();
	...
	case MWIN_EVENT_FINGER_DOWN:
		input->beginTouch(windowEvents->data[0], Vector2(
	...
	case MWIN_EVENT_FINGER_UP:
		input->endTouch(windowEvents->data[0], Vector2(
					(float) windowEvents->data[1] / INT_MAX,
					(float) windowEvents->data[2] / INT_MAX
				));
		break;
	...
	case MWIN_EVENT_FINGER_MOVE:
		input->updateTouch(windowEvents->data[0], input->getLastTouchPosition(windowEvents->data[0]) + Vector2(
					(float) windowEvents->data[1] / INT_MAX,
					(float) windowEvents->data[2] / INT_MAX
				));
		break;
