...
void MES2Context::setClearColor(const Vector4 & color){
...
void MES2Context::setTextureGenMode(TEX_GEN_MODES mode)
{
}

void MES2Context::setTextureFilterMode(TEX_FILTER_MODES min, TEX_FILTER_MODES mag)
{
...
void MES2Context::setTextureUWrapMode(WRAP_MODES wrap)
{
...
void MES2Context::setTextureVWrapMode(WRAP_MODES wrap)
{
	int glWrap = GL_REPEAT;
	if(wrap == WRAP_CLAMP)
		glWrap = GL_CLAMP_TO_EDGE;
...
void MES2Context::setTextureCombineMode(TEX_COMBINE_MODES combine)
{
	/*
...
void MES2Context::sendTextureImage(Image * image, bool mipMap, bool filter, bool compress)
{
	// get properties
...
void MES2Context::texImage(unsigned int level, unsigned int width, unsigned int height, VAR_TYPES type, TEX_MODES mode, const void * pixels)
{
...
void MES2Context::texSubImage(unsigned int level, int xoffset, int yoffset, unsigned int width, unsigned int height, VAR_TYPES type, TEX_MODES mode, const void * pixels)
{
	GLenum format = returnTexMode(mode);
...
void MES2Context::attachFrameBufferTexture(FRAME_BUFFER_ATTACHMENT attachment, unsigned int textureId){
	glFramebufferTexture2D(GL_FRAMEBUFFER, returnAttachType(attachment), GL_TEXTURE_2D, textureId, 0);
}
void MES2Context::setDrawingBuffers(FRAME_BUFFER_ATTACHMENT * buffers, unsigned int size)
{
	/*
     if(size == 0)
     {
...
void MES2Context::sendUniformMatrix(unsigned int fxId, const char * name, Matrix4x4 * matrix, const int count, const bool transpose){
	GLint uValue = glGetUniformLocation(fxId, name);
...
void MES2Context::setVertexPointer(VAR_TYPES type, unsigned int components, const void * pointer)
{
	//int location;
	//getAttribLocation(simpleFX, "Vertex", &location);
...
void MES2Context::setColorPointer(VAR_TYPES type, unsigned int components, const void * pointer)
{
	//int location;
	//getAttribLocation(simpleFX, "Color", &location);
	//setAttribPointer(location, type, components, pointer);
...
void MES2Context::setNormalPointer(VAR_TYPES type, const void * pointer)
{
	//int location;
	//getAttribLocation(simpleFX, "Normal", &location);
...
void MES2Context::setTexCoordPointer(VAR_TYPES type, unsigned int components, const void * pointer)
{
	//int location;
	//getAttribLocation(simpleFX, "TexCoord", &location);
	//setAttribPointer(location, type, components, pointer);
...
void MES2Context::setAttribPointer(unsigned int location, VAR_TYPES type, unsigned int components, const void * pointer, const bool normalized)
{
	glVertexAttribPointer(location, components, returnGLType(type), normalized, 0, pointer);
}

// draw
void MES2Context::drawArray(PRIMITIVE_TYPES type, unsigned int begin, unsigned int size)
{
	glDrawArrays(returnPrimitiveType(type), begin, size);
}

void MES2Context::drawElement(PRIMITIVE_TYPES type, unsigned int size, VAR_TYPES indicesType, const void * indices)
{
...
// material
void MES2Context::setMaterialDiffuse(const Vector4 & diffuse){
	//glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, diffuse);
}
void MES2Context::setMaterialSpecular(const Vector4 & specular){
	//glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, specular);
}
void MES2Context::setMaterialAmbient(const Vector4 & ambient){
	//glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, ambient);
}
void MES2Context::setMaterialEmit(const Vector4 & emit){
...
// color
void MES2Context::setColor(const Color & color)
{
	//glColor4ub(color.r, color.g, color.b, color.a);
}

void MES2Context::setColor4(const Vector4 & color)
{
	//glColor4f(color.x, color.y, color.z, color.w);
}
void MES2Context::setColor3(const Vector3 & color)
{
...
void MES2Context::setDepthMode(DEPTH_MODES mode)
{
	switch(mode)
	{
        case DEPTH_ALWAYS:
...
void MES2Context::setCullMode(CULL_MODES mode)
{
	switch(mode)
	{
        case CULL_FRONT:
...
void MES2Context::setMatrixMode(MATRIX_MODES matrixMode)
{
	m_matrixStep = 0;
...
void MES2Context::multMatrix(const Matrix4x4 * matrix){
	(*m_currentMatrix) *= (*matrix);
}
void MES2Context::translate(const Vector3 & position){ 
	m_currentMatrix->translate(position);
}
void MES2Context::rotate(const Vector3 & axis, float angle){
	m_currentMatrix->rotate(axis, angle);
}
void MES2Context::scale(const Vector3 & scale){
...
void MES2Context::getModelViewMatrix(Matrix4x4 * matrix)
{
	(*matrix) = m_modelViewMatrix;
}
void MES2Context::getProjectionMatrix(Matrix4x4 * matrix)
{
	(*matrix) = m_projectionMatrix;
}
void MES2Context::getTextureMatrix(Matrix4x4 * matrix)
{
...
void MES2Context::setFogColor(const Vector3 & color)
{
...
void MES2Context::getFogColor(Vector3 * color){
	(*color) = m_fogColor;
}
...
void MES2Context::setLightPosition(unsigned int id, const Vector4 & position)
{
	if(id < MAX_MRCLIGHTS)
	{
...
void MES2Context::setLightDiffuse(unsigned int id, const Vector4 & diffuse)
{
	if(id < MAX_MRCLIGHTS)
	{
...
void MES2Context::setLightSpecular(unsigned int id, const Vector4 & specular)
{
	if(id < MAX_MRCLIGHTS)
	{
...
void MES2Context::setLightAmbient(unsigned int id, const Vector4 & ambient)
{
	if(id < MAX_MRCLIGHTS)
	{
...
void MES2Context::setLightSpotDirection(unsigned int id, const Vector3 & direction)
{
	if(id < MAX_MRCLIGHTS)
	{
...
void MES2Context::getLightPosition(unsigned int id, Vector4 * position)
{
	if(id < MAX_MRCLIGHTS)
		(*position) = m_lights[id].position;
}
void MES2Context::getLightDiffuse(unsigned int id, Vector4 * diffuse)
{
	if(id < MAX_MRCLIGHTS)
		(*diffuse) = m_lights[id].diffuse;
}
void MES2Context::getLightSpecular(unsigned int id, Vector4 * specular)
{
	if(id < MAX_MRCLIGHTS)
		(*specular) = m_lights[id].specular;
}
void MES2Context::getLightAmbient(unsigned int id, Vector4 * ambient)
{
	if(id < MAX_MRCLIGHTS)
...
void MES2Context::getLightSpotDirection(unsigned int id, Vector3 * direction)
{
	if(id < MAX_MRCLIGHTS)
...
void MES2Context::setBlendingMode(BLENDING_MODES mode)
{
	switch(mode)
	{
        case BLENDING_NONE:
