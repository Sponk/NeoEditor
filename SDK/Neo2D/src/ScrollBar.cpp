...
#define KNOB_WIDTH (m_direction == SLIDER_VERTICAL)? MAX(10.0, m_height * (m_height / m_range.y)) : MAX(10.0, m_width * (m_width / m_range.y))
...
void ScrollBar::update()
{
	...
	{
		knobpos = Vector2(offset.x + (m_value/m_range.y) * m_width, offset.y);
		knobsize = Vector2(KNOB_WIDTH, m_height * KNOB_HEIGHT);
	}
	else
	{
		knobpos = Vector2(offset.x,  offset.y + (m_value/m_range.y) * m_height);
	...
	{
		m_state = SLIDER_GRABBED;
		m_value = MAX(0, MIN(m_range.y-length, (m_range.y/length)*delta + m_value));
		doCallback();
	}
...
void ScrollBar::draw(Vector2 offset)
{
	...
	render->drawColoredQuad(knobpos.x, knobpos.y, knobsize.x, knobsize.y, Vector4(0.5,0.5,0.5,1));
}
