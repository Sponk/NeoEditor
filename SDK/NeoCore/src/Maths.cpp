...
bool isEdgeToEdge2dIntersection(const Vector2 & A, const Vector2 & B, const Vector2 & C, const Vector2 & D, Vector2 * I)
{
	Vector2 DP, QA, QB;
	float d, la, lb;
...
bool isLineCircleIntersection(const Vector2 & origin, const Vector2 & dest, const Vector2 & circleCenter, float circleRadius)
{
	...
	Vector2 vec = dest - origin;
	Vector2 N = Vector2(-vec.y, vec.x).getNormalized();
...
bool isBoxToBoxCollision(const Vector3 & minA, const Vector3 & maxA, const Vector3 & minB, const Vector3 & maxB)
{
	// X
...
bool isBoxToBox2dCollision(const Vector2 & minA, const Vector2 & maxA, const Vector2 & minB, const Vector2 & maxB)
{
	// X
	if(minA.x > maxB.x) return false;
	if(maxA.x < minB.x) return false;
...
bool isPointInBox(const Vector3 & point, const Vector3 & min, const Vector3 & max)
{
...
bool isPointInBox2d(const Vector2 & point, const Vector2 & min, const Vector2 & max)
{
	if(point.x > max.x)
		return false;
...
bool isEdgeToBoxCollision(const Vector3 & origin, const Vector3 & dest, const Vector3 & min, const Vector3 & max)
{
	Vector3 nrm[6];
	Vector3 pts[6];
	Vector3 pt;
...
bool isEdgePlaneIntersection(const Vector3 & origin, const Vector3 & dest, const Vector3 & planePoint, const Vector3 & normal, Vector3 * point)
{
	Vector3 vec_tmp[2], vec[2];
...
bool isRaySphereIntersection(const Vector3 & origin, const Vector3 & direction, const Vector3 & sphereCenter, float sphereRadius, Vector3 * point)
{
	Vector3 vec = origin - sphereCenter;
...
bool isRayPlaneIntersection(const Vector3 & origin, const Vector3 & direction, const Vector3 & planePoint, const Vector3 & planeNormal, Vector3 * point)
{
...
bool isPointInTriangle(const Vector3 & point, const Vector3 & a, const Vector3 & b, const Vector3 & c, const Vector3 & normal)
{
	Vector3 nrm = getTriangleNormal(point, a, b);
...
Vector3 RGBToHSV(Vector3 rgbColor)
{
	float r = rgbColor.x;
	float g = rgbColor.y;
	float b = rgbColor.z;
...
Vector3 HSVToRGB(Vector3 HSVColor)
{
	int i;
	float r, g, b;
	float f, p, q, t;
...
Vector3 RGBToHSL(Vector3 rgbColor)
{
...
Vector3 HSLToRGB(Vector3 hslColor)
{
	float H = hslColor.x;
	float S = hslColor.y;
	float L = hslColor.z;
...
void simplifyDP(float tol, Vector3 * v, int j, int k, int * mk)
{
    ...
    Vector3	S[2] = {v[j], v[k]};		// segment from v[j] to v[k]
    Vector3	u = S[1] - S[0];			// segment direction vector
    ...
    Vector3 w;
    Vector3 Pb;               // base of perpendicular from v[i] to S
