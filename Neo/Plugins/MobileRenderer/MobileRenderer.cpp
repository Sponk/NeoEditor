...
MobileRenderer::MobileRenderer(void):
m_fboId(0),
m_forceNoFX(false),
m_verticesNumber(0),
m_normalsNumber(0),
...
void MobileRenderer::initialize()
{
...
void MobileRenderer::destroy(void)
{
	delete this;
}

Renderer* MobileRenderer::getNew(void)
{
	return new MobileRenderer();
}

void MobileRenderer::addFX(const char * vert, const char * frag)
{
...
Vector3 * MobileRenderer::getVertices(unsigned int size)
{
	if(size == 0)
		return NULL;
...
Vector3 * MobileRenderer::getNormals(unsigned int size)
{
	if(size == 0)
		return NULL;
...
Vector3 * MobileRenderer::getTangents(unsigned int size)
{
	if(size == 0)
		return NULL;
...
void MobileRenderer::updateSkinning(Mesh * mesh, Armature * armature)
{
	unsigned int s;
...
void MobileRenderer::initVBO(SubMesh * subMesh)
{
...
void MobileRenderer::drawDisplay(SubMesh * subMesh, MaterialDisplay * display, Vector3 * vertices, Vector3 * normals, Vector3 * tangents, Color * colors)
{
	NeoEngine * engine = NeoEngine::getInstance();
	RenderingContext * render = engine->getRenderingContext();
...
void MobileRenderer::drawOpaques(SubMesh * subMesh, Armature * armature)
{
	// data
	Vector3 * vertices = subMesh->getVertices();
	Vector3 * normals = subMesh->getNormals();
...
void MobileRenderer::drawTransparents(SubMesh * subMesh, Armature * armature)
{
	RenderingContext * render = NeoEngine::getInstance()->getRenderingContext();
...
float MobileRenderer::getDistanceToCam(OCamera * camera, const Vector3 & pos)
{
	if(! camera->isOrtho())
	{
...
void MobileRenderer::setShadowMatrix(Matrix4x4 * matrix, OCamera * camera)
{
	NeoEngine * engine = NeoEngine::getInstance();
	RenderingContext * render = engine->getRenderingContext();
...
void MobileRenderer::updateVisibility(Scene * scene, OCamera * camera)
{
	// make frustum
...
void MobileRenderer::enableFog(OCamera * camera)
{
	RenderingContext * render = NeoEngine::getInstance()->getRenderingContext();
...
ShadowLight * MobileRenderer::createShadowLight(OLight * light)
{
	RenderingContext * render = NeoEngine::getInstance()->getRenderingContext();
...
void MobileRenderer::destroyUnusedShadowLights(void)
{
	RenderingContext * render = NeoEngine::getInstance()->getRenderingContext();
...
void MobileRenderer::decreaseShadowLights(void)
{
	// keys
	map<unsigned long, ShadowLight>::iterator
	mit (m_shadowLights.begin()),
...
void MobileRenderer::drawText(OText * textObj)
{
	RenderingContext * render = NeoEngine().getInstance()->getRenderingContext();
...
void MobileRenderer::prepareSubMesh(Scene * scene, OCamera * camera, OEntity * entity, SubMesh * subMesh)
{
	RenderingContext * render = NeoEngine::getInstance()->getRenderingContext();
...
void MobileRenderer::drawScene(Scene * scene, OCamera * camera)
{
	// get render
	RenderingContext * render = NeoEngine::getInstance()->getRenderingContext();
...
const char* m_colorOnlyFragShader = //"#version 300 es\n"
									"#ifdef GL_ES\n"
										"precision mediump float;\n"
									"#endif\n"
									"uniform vec4 color;"
...
void MobileRenderer::drawColoredQuad(const Vector2& position, const Vector2& size, const Vector4& color, float rotation)
{
	...
	m_vertices[0] = Vector2(position.x, position.y);
	m_vertices[1] = Vector2(position.x, position.y + size.y);
	m_vertices[2] = Vector2(position.x + size.x, position.y);
	m_vertices[3] = Vector2(position.x + size.x, position.y + size.y);
	...
	// So we don't cast away const-ness
	float w = size.x;
	float h = size.y;
...
void MobileRenderer::drawTexturedQuad(const Vector2& position, const Vector2& size, int texture, float rotation)
{
	drawTexturedQuad(position, size, texture, rotation, Vector2(1, 1),
		Vector2(1, 1), Vector4(0.0, 0.0, 1.0, 1.0));
}

void MobileRenderer::drawTexturedQuad(const Vector2& position, const Vector2& size, int texture,
	...
	float rotation, const Vector2& scale, const Vector2& flip, const Vector4& texcoords)
{
	...
	if (flip.x != 0 || flip.y != 0)
	{
		...
		{
			m_left = position.x;
			m_right = position.x + (size.x * flip.x);
		}
		else
		{
			m_right = position.x;
			m_left = position.x + (size.x * -flip.x);
		}
		if (flip.y > 0)
		{
			m_top = position.y;
			m_bottom = position.y + (size.y * flip.y);
		}
		else
		{
			m_bottom = position.y;
			m_top = position.y + (size.y * -flip.y);
		}
...
void MobileRenderer::set2D(const Vector2& size)
{
	RenderingContext* render = NeoEngine::getInstance()->getRenderingContext();
...
void MobileRenderer::loadShader(const char* vert, const char* frag, unsigned int* fx)
{
	RenderingContext* render = NeoEngine::getInstance()->getRenderingContext();
...
void MobileRenderer::drawText2D(OText* text, float x, float y, float rotation)
{
